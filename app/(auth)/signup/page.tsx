...
export default function SignupPage() {
  ...
  // Check URL param on mount — ?role=operator skips the choose screen
  useEffect(() => {
  ...
  async function handleOperatorSignup() {
    ...
    // Create the truck record immediately so it's ready after confirmation
    if (data.user) {
      try {
        ...
        // Non-fatal — operator can complete profile from dashboard
      }
    }
    ...
    return (
      <div className="min-h-screen bg-neutral-50 flex flex-col items-center justify-center p-6">
        <div className="mb-8 text-center">
          ...
          <p className="text-neutral-500 text-sm">Join the community</p>
        </div>

        <div className="w-full max-w-sm flex flex-col gap-3">
          {/* Operator card — prominent */}
          <button
          ...
          >
            ...
            <p className="text-neutral-400 text-sm">
              Get on the map, go live in 1 tap, get discovered
            </p>
          ...
          >
            <p className="text-neutral-800 font-bold text-base mb-1">
              Find food trucks near me
            </p>
            <p className="text-neutral-400 text-sm">
              Follow your favorites, get notified when they go live
            </p>
          </button>
        ...
        <div className="max-w-sm mx-auto px-6 py-10">
          ...
          <div className="flex gap-2 mb-7 flex-wrap">
            {[
              ...
              { icon: "live", label: "Go live in 1 tap" },
              { icon: "map", label: "Real-time map" },
            ].map(({ icon, label }) => (
              <div key={icon} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-neutral-200 rounded-full">
              ...
              <div className="flex flex-col gap-3">
                <div>
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full px-4 py-3 rounded-xl border border-neutral-200 text-sm mt-1.5 focus:outline-none focus:border-brand-red transition-colors"
                  />
                </div>
  ...
  if (step === "customer") {
    return (
      <div className="min-h-screen bg-neutral-50 flex flex-col items-center justify-center p-6">
        ...
        <button
          onClick={() => setStep("choose")}
          className="self-start flex items-center gap-1.5 text-neutral-400 text-sm mb-6 hover:text-neutral-600 transition-colors"
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
            <path d="M19 12H5M12 5l-7 7 7 7"/>
          </svg>
      ...
      <div className="min-h-screen bg-neutral-50 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm flex flex-col gap-4">
          ...
          <div className="text-center mb-2">
            ...
            <p className="text-neutral-500 text-sm mt-1">
              One last step to get on the map.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: `We sent a link to ${opEmail} — click it to activate your account.`,
                active: true,
              },
              {
                num: "2",
                title: "Sign in to your dashboard",
                desc: "Complete your profile, add menu items, and set your schedule.",
                active: false,
              },
              {
                num: "3",
                title: "Tap Go Live",
                desc: "One tap and your truck appears on the map instantly.",
                active: false,
              },
            ].map((item, i) => (
              <div
                key={item.num}
                className={`flex gap-4 px-5 py-4 ${i < 2 ? "border-b border-neutral-100" : ""}`}
              >
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 mt-0.5 ${
                  item.active ? "bg-brand-red text-white" : "bg-neutral-100 text-neutral-400"
                }`}>
                  {item.num}
                </div>
                <div>
                  <p className={`text-sm font-bold ${item.active ? "text-neutral-900" : "text-neutral-500"}`}>
                    {item.title}
                  </p>
