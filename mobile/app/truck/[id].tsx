    ...
    async function load() {
      try {
        const [truckRes, menuRes, locationRes, spottedRes, reviewsRes] = await Promise.all([
          supabase.from('trucks').select('id, name, cuisine, description, profile_photo, is_live, dietary_tags, instagram, phone, avg_rating, review_count, catering_description, catering_starting_price, catering_min_guests').eq('id', id).maybeSingle(),
