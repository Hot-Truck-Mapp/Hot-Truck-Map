...
DROP POLICY IF EXISTS "trucks_owner_insert" ON trucks;
DROP POLICY IF EXISTS "trucks_owner_update" ON trucks;
DROP POLICY IF EXISTS "trucks_owner_delete" ON trucks;
...
CREATE POLICY "locations_owner_write"  ON locations FOR INSERT WITH CHECK (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "locations_owner_update" ON locations FOR UPDATE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "locations_owner_delete" ON locations FOR DELETE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
...
CREATE POLICY "menu_items_owner_write"  ON menu_items FOR INSERT WITH CHECK (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "menu_items_owner_update" ON menu_items FOR UPDATE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "menu_items_owner_delete" ON menu_items FOR DELETE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);

-- Schedules: public read, truck owner write
DROP POLICY IF EXISTS "schedules_public_read"  ON schedules;
DROP POLICY IF EXISTS "schedules_owner_write"  ON schedules;
...
CREATE POLICY "schedules_owner_write"  ON schedules FOR INSERT WITH CHECK (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "schedules_owner_update" ON schedules FOR UPDATE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "schedules_owner_delete" ON schedules FOR DELETE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
...
DROP POLICY IF EXISTS "catering_req_owner_read"   ON catering_requests;
DROP POLICY IF EXISTS "catering_req_owner_update" ON catering_requests;
CREATE POLICY "catering_req_insert"       ON catering_requests FOR INSERT WITH CHECK (true);
...
DROP POLICY IF EXISTS "catering_pkg_public_read"  ON catering_packages;
DROP POLICY IF EXISTS "catering_pkg_owner_insert" ON catering_packages;
DROP POLICY IF EXISTS "catering_pkg_owner_update" ON catering_packages;
DROP POLICY IF EXISTS "catering_pkg_owner_delete" ON catering_packages;
CREATE POLICY "catering_pkg_public_read"  ON catering_packages FOR SELECT USING (true);
CREATE POLICY "catering_pkg_owner_insert" ON catering_packages FOR INSERT WITH CHECK (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "catering_pkg_owner_update" ON catering_packages FOR UPDATE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
CREATE POLICY "catering_pkg_owner_delete" ON catering_packages FOR DELETE USING (
  auth.uid() = (SELECT owner_id FROM trucks WHERE id = truck_id)
);
...
CREATE POLICY "reviews_auth_delete"  ON reviews FOR DELETE USING (auth.uid() = user_id);
